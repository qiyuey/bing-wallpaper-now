...
- 优化 `index.json` 文件大小（短字段名 + 紧凑格式）
- 改进代码质量，删除未使用的代码和 `#[allow(dead_code)]` 属性
- 所有测试通过（Rust 55 个测试 + 前端 128 个测试）
...
- ✨ **按需下载策略**
  - 实现图片按需下载机制，只在真正需要时（查看或设置壁纸）才下载图片
  - 更新循环和索引重建时只更新元数据，不批量下载图片
  - JPG 文件不区分语言，一次下载后不再重复下载，节省带宽和存储空间
  - 新增 `download_wallpaper_if_needed` 函数，支持按需下载单个壁纸

- 🔒 **路径安全性检查**
  - 添加路径验证逻辑，防止路径遍历攻击
  - 验证文件路径是否在壁纸目录下，确保安全性

### Changed

- 🔧 **更新逻辑优化**
  - 移除更新循环中的批量下载逻辑，改为按需下载策略
  - 优化索引重建和语言切换场景，只更新元数据，不下载图片
  - 设置壁纸时自动检查文件是否存在，不存在则按需下载
  - 自动应用最新壁纸时，如果文件不存在会自动下载

### Fixed

- 🐛 **修复图片显示损坏问题**
  - 修复图片下载完成后在页面中显示损坏的问题
  - 下载完成后自动清除缓存，强制浏览器重新加载图片
  - 在图片 URL 中添加重试计数参数，避免浏览器使用缓存的损坏图片

- 🐛 **修复按需下载场景下的壁纸设置失败**
  - 修复按需下载策略下，自动应用最新壁纸可能因文件不存在而失败的问题
  - 在设置壁纸前检查文件是否存在，不存在则自动下载
