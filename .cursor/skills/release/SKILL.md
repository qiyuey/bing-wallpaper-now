...
1. 确认发布范围与工作区状态
   - 本流程**仅支持正式版发布**（`make release`），不做其它模式。
   - 查看当前版本（应为 `X.Y.Z-0` 开发版本）。
   - **若当前版本不是开发版本**（无 `-0` 后缀），则先按 `.cursor/skills/bump-version/SKILL.md` 执行版本升级：以选项方式让用户选择 patch/minor/major，完成 bump 及依赖更新后，再继续本发布流程的后续步骤。
   - 对比最近 tag 到 `HEAD` 的差异，确认本次发布内容边界。
   - 确认工作区无未提交变更。若有未提交变更，先按 `~/.cursor/skills/commit/SKILL.md` 执行提交，再继续本流程。
...
4. **人工确认发布内容（必须）**
   - 将完整的 CHANGELOG 新增内容展示给用户；若步骤 3 中更新了文档，一并展示变更摘要
   - **必须**以选项形式请求确认，让用户通过点击选项回答：
     - **确认无误，继续提交和发布** — 进入步骤 5
     - **需要修改 CHANGELOG** — 按用户反馈修正后再次展示并请求确认
     - **需要修改文档** — 按用户反馈修正后再次展示并请求确认
     - **取消发布** — 停止流程，不提交、不执行 `make release`
   - 说明文案示例：「请确认 CHANGELOG 及文档更新内容，并点击上方选项之一。」
   - **必须等待用户选择其中一项后才能进入下一步**；若用户选择修改，修正后再次以选项形式请求确认，直到用户选择「确认无误」或「取消发布」
