...
#
# 用法：
#   ./scripts/version-maven.sh snapshot-patch  # 创建下一个 patch SNAPSHOT (0.1.0 -> 0.1.1-SNAPSHOT)
#   ./scripts/version-maven.sh snapshot-minor  # 创建下一个 minor SNAPSHOT (0.1.0 -> 0.2.0-SNAPSHOT)
#   ./scripts/version-maven.sh snapshot-major  # 创建下一个 major SNAPSHOT (0.1.0 -> 1.0.0-SNAPSHOT)
#   ./scripts/version-maven.sh release         # 发布当前 SNAPSHOT 版本 (0.1.1-SNAPSHOT -> 0.1.1)
#   ./scripts/version-maven.sh release-push    # 发布并推送 tag 到远程
#
# 工作流程：
#   1. 发布 v0.1.0 后，创建 0.1.1-SNAPSHOT 用于开发
#   2. 开发完成后，运行 release 转为 0.1.1 正式版本并打 tag
...
release_version() {
    ...
    echo ""
    echo "  3. 发布后，创建下一个 SNAPSHOT 版本:"
    echo "     make snapshot-patch  # 或 snapshot-minor, snapshot-major"
    echo ""
...
main() {
    ...
    if [ $# -eq 0 ]; then
        ...
        echo "  $0 release             # 发布当前 SNAPSHOT 版本"
        echo "  $0 release-push        # 发布并推送到远程"
        echo ""
        exit 0
    fi
    ...
    case "$1" in
        ...
        *)
            print_error "未知命令: $1"
            print_info "用法: $0 <snapshot-patch|snapshot-minor|snapshot-major|release|release-push|info>"
            exit 1
            ;;
    esac
}
